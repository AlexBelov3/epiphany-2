                ...
                with torch.no_grad():
                    ...
                    if NUM_Vs > 1:
                        for j in range(NUM_Vs):
                            y_hat_L_list.append(torch.tensor(np.array(y_hat.cpu().squeeze())[j][:100]).unsqueeze(0))
                            y_hat_R_list.append(torch.tensor(np.array(y_hat.cpu().squeeze())[j][100:]).unsqueeze(0))
