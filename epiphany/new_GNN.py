...
class EdgeWeightMPNN(MessagePassing):
    def __init__(self, track_channels, track_length, hidden_dim, edge_dim):
        ...
        self.linear = nn.Linear(1 * (track_length // 2) + 1, hidden_dim)
    ...
    def message(self, x_i, x_j, edge_attr):
        ...
        edge_weights = self.edge_weight_predictor(msg_input).squeeze(-1)
        print(f"edge_weights shape: {edge_weights.shape}")

        # Transform the message
        transformed_msg = self.message_mlp(msg_input)
        return edge_weights.unsqueeze(-1) * transformed_msg
    ...
    def predict_edge_weights(self, x, edge_index):
        ...
        edge_weights = self.edge_weight_predictor(edge_embeddings)
        print(f"edge_weights shape: {edge_weights.shape}")
