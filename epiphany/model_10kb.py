...
class branch_outer_prod_big(nn.Module):
    def __init__(self):
        ...
        # )
        self.classifier2 = nn.Sequential(
            nn.Linear(in_features=(392), out_features=200))  # (nn.Linear(in_features=(512), out_features=200))
...
class branch_cov_2d(nn.Module):
    def __init__(self):
        ...
        self.cov_extractor = nn.Sequential(
            nn.Conv2d(
                in_channels=1, out_channels=16, kernel_size=(1,5), stride=1, padding=(1,2)
            ),
            nn.BatchNorm2d(16),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2),
            nn.Conv2d(
                in_channels=16, out_channels=16, kernel_size=(1,5), stride=1, padding=(1,2)
            ),
            nn.BatchNorm2d(16),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2),
