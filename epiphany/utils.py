...
def generate_image_test(label, y_up_list, y_down_list, path='./', seq_length=200, num_vs = 1):
    ...
    print(f"y_up_list len (before): {y_up_list}")
    y_up_list = torch.cat(y_up_list, dim=0)
    print(f"y_up_list len (after): {y_up_list}")
    print(f"y_down_list len (before): {y_down_list}")
    y_down_list = torch.cat(y_down_list, dim=0)
    print(f"y_down_list len (after): {y_down_list}")
