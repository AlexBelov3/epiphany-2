...
class Chip2HiCDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        contact_data = []
        print(f"range(idx + self.buf, np.minimum(idx + self.seq_length + self.buf, len(self.labels[chr][0]) - self.buf),1): {range(idx + self.buf, np.minimum(idx + self.seq_length + self.buf, len(self.labels[chr][0]) - self.buf),1)}")
        print(f"range(idx * self.num_Vs + self.buf, np.minimum(idx * self.num_Vs + self.seq_length + self.buf, len(self.labels[chr][0]) - self.buf), 1): {range(idx * self.num_Vs + self.buf, np.minimum(idx * self.num_Vs + self.seq_length + self.buf, len(self.labels[chr][0]) - self.buf), 1)}")
